...
import { TrendingUp, TrendingDown, Minus, Plus, Upload, Download, Trash2, Calculator, ChevronDown, ChevronUp, AlertTriangle, CheckCircle2, Circle, X, Search, Save, FileSpreadsheet, Settings, Wifi, WifiOff, RefreshCw, Loader2 } from "lucide-react";
      ...
      <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] overflow-y-auto shadow-2xl">
        ...
        <div className="p-5">
          <div className="flex gap-1 bg-stone-100 rounded-lg p-1 mb-4">
            <button onClick={() => setTab("paste")} className={`flex-1 py-1.5 rounded-md text-xs font-medium ${tab === "paste" ? "bg-white shadow-sm text-stone-800" : "text-stone-500"}`}>Paste CSV</button>
            <button onClick={() => setTab("file")} className={`flex-1 py-1.5 rounded-md text-xs font-medium ${tab === "file" ? "bg-white shadow-sm text-stone-800" : "text-stone-500"}`}>Upload File</button>
          </div>
          ...
          <div className="flex gap-2 mt-4">
            <button onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-stone-200 text-stone-600 text-sm font-medium">Cancel</button>
            <button onClick={doImport} disabled={!text.trim()} className="flex-1 py-2.5 rounded-lg bg-teal-600 text-white text-sm font-medium disabled:opacity-40">
              Import
            </button>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-stone-50 font-sans" style={{ fontFamily: "'Inter', -apple-system, sans-serif" }}>
      ...
      <div className="bg-gradient-to-br from-teal-700 to-teal-900 text-white px-5 pt-6 pb-8 rounded-b-3xl shadow-lg">
        ...
        <p className="text-teal-100 text-xs">NSE · BSE — auto-detected from code {apiKey ? "· Live data connected" : "· Tap the wifi icon to connect live data"}</p>
